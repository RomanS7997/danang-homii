...
export function App() {
  ...
  return <div className={`design-shell design-${visualStyle} ${designPreview ? 'is-design-preview' : ''}`}>
    <header className="site-header">
      ...
      <div className="header-right"><label className="language-picker"><span className="sr-only">{t('Язык сайта')}</span><select aria-label={t('Язык сайта')} value={lang} onChange={e => changeLanguage(e.target.value)}><option value="ru" lang="ru">RU</option><option value="en" lang="en">EN</option><option value="vi" lang="vi">VI</option></select></label><button className="navy-button header-cta" onClick={() => request()}>{t("Подобрать жильё")}</button><button className="round-button menu-button" aria-label={t(menu ? 'Закрыть меню' : 'Открыть меню')} aria-expanded={menu} aria-controls="site-navigation" onClick={() => setMenu(!menu)}><ListIcon size={25} /></button></div>
    </header>
