...
export function App() {
  ...
  return <div className={`design-shell design-${visualStyle} ${designPreview ? 'is-design-preview' : ''}`}>
    <header className="site-header" onKeyDown={e => { if (e.key === 'Escape' && menu) { setMenu(false); document.querySelector('.menu-button')?.focus(); } }}>
      ...
      <nav id="site-navigation" className={menu ? 'nav-open' : ''} aria-label={t("Главная навигация")} onKeyDown={e=>{if(e.key === 'Escape'){setMenu(false);document.querySelector('.menu-button')?.focus();}}}>{[['apartments', 'Квартиры'], ['districts', 'Районы'], ['how-to-rent', 'Как снять'], ['guides', 'Полезное'], ['about', 'О нас']].map(([to, label]) => <Link key={to} to={to} aria-current={page === to || page.startsWith(`${to}/`) ? 'page' : undefined}>{t(label)}</Link>)}<button className="mobile-menu-request" onClick={()=>{setMenu(false);request();}}>{t("Подобрать жильё")}<ArrowRightIcon size={18}/></button></nav>
      <div className="header-right"><label className="language-picker"><span className="sr-only">{t('Язык сайта')}</span><select aria-label={t('Язык сайта')} value={lang} onChange={e => changeLanguage(e.target.value)}><option value="ru" lang="ru">RU</option><option value="en" lang="en">EN</option><option value="vi" lang="vi">VI</option></select></label><button className="navy-button header-cta" onClick={() => request()}>{t("Подобрать жильё")}</button><button className="round-button menu-button" aria-label={t(menu ? 'Закрыть меню' : 'Открыть меню')} aria-expanded={menu} aria-controls="site-navigation" onClick={() => setMenu(!menu)}>{menu ? <XIcon size={23} /> : <ListIcon size={23} />}</button></div>
    </header>
