...
import { FaqSection } from './LandingSections.jsx';
import { assetPath } from './paths.js';
import { copy } from './enrichment-content.js';
import { DistrictExplorer, EditorialFigure, MovingJourney, ViewingChecklist, BudgetPlanner, AreaCallout, LifestyleStandards, OwnerPreparation } from './ExploreTools.jsx';
...
export function PageIntro({ eyebrow, title, description, art, children }) {
  const { t } = useLocale();
  return <section className={`page-intro ${art ? 'with-intro-art' : ''} ${art?.startsWith('editorial/')?'editorial-intro':''}`}>
    <div><p className="eyebrow">{t(eyebrow)}</p><h1>{t(title)}</h1><p className="page-intro-description">{t(description)}</p>{children}</div>
    {art && (art.startsWith('editorial/') ? <EditorialFigure image={art.slice(10)} className="page-intro-art" priority/> : <img src={assetPath(`${art}.webp`)} alt="" className="page-intro-art"/>)}
  </section>;
}
...
export function PropertyPage({ home, homes, favorites, onFavorite, onMap, onRequest }) {
  ...
  return <div className="inner-page property-page">
    ...
    <AreaCallout areaId={areaIds[areaNames.indexOf(home.district)]}/><section className="related-homes"><div className="editorial-heading"><h2>{t('Ещё немного вдохновения.')}</h2><Link to="apartments" className="text-button">{t('Все квартиры')}<ArrowRightIcon size={19}/></Link></div><div className="home-grid">{homes.filter(h=>h.id!==home.id).slice(0,3).map(home=><HomeCard key={home.id} {...{home,favorites,onFavorite,onMap}}/>)}</div></section>
  </div>;
}
export function DistrictsPage({ onRequest }) {
  return <div className="inner-page districts-page enriched-page"><PageIntro eyebrow={copy.districtEyebrow} title={copy.districtTitle} description={copy.districtIntro}/><DistrictExplorer/><HelpBand onRequest={onRequest}/></div>;
}
export function RentalPage({ onRequest }) {
  return <div className="inner-page rental-page enriched-page"><PageIntro eyebrow="СПОКОЙНО. ПО ШАГАМ." title={copy.rentTitle} description={copy.rentIntro} art="editorial/moving-day-v1"/><MovingJourney/><ViewingChecklist/><BudgetPlanner/><FaqSection onRequest={onRequest}/><HelpBand onRequest={onRequest}/></div>;
}

export const guides = [
  {id:'choose-your-neighbourhood',title:'Как выбрать район под свой ритм',category:'Районы',image:'editorial/an-thuong-v1',intro:'Начните с привычек, а не с названия района. Простой способ понять, где вам будет удобно.',sections:[
  ...
  {id:'viewing-checklist',title:'Что проверить на просмотре квартиры',category:'Просмотр',image:'editorial/owners-home-v1',intro:'Свет, тишина и бытовые детали. Сохраните этот список перед личным или видеопросмотром.',sections:[
  ...
  {id:'moving-in',title:'Первые дни в новом доме',category:'Заселение',image:'editorial/moving-day-v1',intro:'Несколько простых действий, которые помогут спокойно освоиться после переезда.',sections:[
...
export function GuidesPage() {
  ...
  return <div className="inner-page guides-page enriched-page"><PageIntro eyebrow={copy.guidesEyebrow} title={copy.guidesTitle} description={copy.guidesIntro} art="editorial/city-notes-v1"/><div className="guide-topic-filters" role="group" aria-label={t(copy.allTopics)}>{['all',...guides.map(g=>g.category)].map(value=><button key={value} aria-pressed={value===category} onClick={()=>setCategory(value)}>{t(value==='all'?copy.allTopics:value)}</button>)}</div><div className="guide-grid">{guides.filter(g=>category==='all'||g.category===category).map((guide)=><Link to={`guides/${guide.id}`} className="guide-card" key={guide.id}><div className="guide-art"><img src={assetPath(`${guide.image}.webp`)} alt="" loading="lazy"/><span>{t(copy.toolInside)}</span></div><div className="guide-copy"><p className="eyebrow">{t(guide.category)}</p><h2>{t(guide.title)}</h2><p>{t(guide.intro)}</p><span className="text-button">{t('Читать материал')}<ArrowUpRightIcon size={20}/></span></div></Link>)}</div><AreaCallout/></div>;
}
export function GuidePage({id,onRequest}) {
  const {t}=useLocale();
  const guide=guides.find(g=>g.id===id);
  if(!guide)return <NotFound/>;
  return <div className="inner-page article-page enriched-page"><Link to="guides" className="text-button article-back"><ArrowLeftIcon size={18}/>{t('Все материалы')}</Link><PageIntro eyebrow={guide.category} title={guide.title} description={guide.intro} art={guide.image}><a className="text-button guide-tool-jump" href="#guide-tool">{t(copy.toolInside)}<ArrowRightIcon size={18}/></a></PageIntro><div className="article-layout"><aside><p className="eyebrow">{t('В ЭТОМ МАТЕРИАЛЕ')}</p>{guide.sections.map(([title],i)=><a key={title} href={`#part-${i}`}>{String(i+1).padStart(2,'0')} · {t(title)}</a>)}</aside><article>{guide.sections.map(([title,body],i)=><section id={`part-${i}`} key={title}><p className="eyebrow">0{i+1}</p><h2>{t(title)}</h2><p>{t(body)}</p></section>)}</article></div><div id="guide-tool">{id==='viewing-checklist'?<ViewingChecklist/>:id==='moving-in'?<MovingJourney initial={3}/>:<AreaCallout/>}</div><HelpBand onRequest={onRequest}/></div>;
}
export function AboutPage({onRequest}) {
  const {t}=useLocale();
  return <div className="inner-page about-page enriched-page"><PageIntro eyebrow="МЫ — DANANG HOMII" title={copy.aboutTitle} description={copy.aboutIntro}/><section className="about-story"><EditorialFigure image="local-perspective-v1" priority/><div><p className="eyebrow">{t('МЕСТНЫЙ ВЗГЛЯД. ПОНЯТНЫЙ ПОДХОД.')}</p><h2>{t('За красивой фотографией — повседневная жизнь.')}</h2><p>{t('Нам важно, что будет после просмотра: удобно ли работать, легко ли добираться, подходит ли вам окружение. Поэтому обсуждаем не только метры и цену, но и ваш обычный день.')}</p>{[['Внимание к деталям','Состояние квартиры, расположение и условия аренды — разбираем до решения.'],['Понятный разговор','Объясняем варианты и отвечаем на вопросы без давления.'],['Помощь с переездом','Согласуем просмотр и заселение, помогаем разобраться с бытовыми вопросами.']].map(([h,p])=><div className="about-principle" key={h}><CheckIcon size={21}/><div><h3>{t(h)}</h3><p>{t(p)}</p></div></div>)}</div></section><LifestyleStandards/><section className="contact-block" id="contacts"><div><p className="eyebrow">{t('ДАВАЙТЕ ПОЗНАКОМИМСЯ')}</p><h2>{t('Ваш дом начинается с разговора.')}</h2><p>{t('На русском, английском и вьетнамском.')}</p><p className="contact-location"><MapPinIcon size={20}/>Đà Nẵng, Việt Nam</p></div><div><p>{t('Укажите пожелания и удобный способ связи. Так мы сможем начать с того, что для вас действительно важно.')}</p><button className="navy-button" onClick={()=>onRequest()}>{t('Обсудить подбор')}<ArrowRightIcon size={20}/></button><Link to="owners" className="text-button">{t('Я собственник жилья')}<ArrowUpRightIcon size={18}/></Link></div></section></div>;
}
